...
map = L.map('map', {
    ...
    crs: L.CRS.EPSG4326,
    center: [-7.371149562356164, 109.96209629162517],
    zoom: 12,
    zoomControl: false
...
function legend() {
	...
	var head = document.createElement("p");
        ...
        element.appendChild(head);
	 var img = new Image();
	  img.src = "http://localhost:8084/geoserver/GISKertek/wms?REQUEST=GetLegendGraphic&VERSION=1.0.0&FORMAT=image/png&WIDTH=20&HEIGHT=20&LAYER=" +layer.options.layers;
...
$(document).ready(function() {
    $.ajax({
        type: "GET",
        url: "http://localhost:8084/geoserver/GISKertek/wfs?request=getCapabilities",
        dataType: "xml",
...
function wms_layers() {
  ...
  $("#wms_layers_window").modal('show');
    ...
    $(document).ready(function() {
        $.ajax({
            type: "GET",
            url: "http://localhost:8084/geoserver/GISKertek/wms?request=getCapabilities",
            dataType: "xml",
            success: function(xml) {
...
function clear_all() {
    ...
    map.flyTo([-7.367562943599829, 109.96708118496095], 12);
